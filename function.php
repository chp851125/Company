...
function add_work($title, $introduce, $image_path, $video_path, $publish){
    $result = null;
    ...
    $sql= "INSERT INTO work (title, introduce, image_path, video_path, publish, upload_date, create_userid)
    VALUES('{$title}', '{$introduce}', {$image_path_value}, {$video_path_value}, '{$publish}', '{$upload_date}', '{$create_userid}')";
    $query = $_SESSION['$link']->query($sql);
    //echo ($sql);
    if($query){
        if(mysqli_affected_rows($_SESSION['$link']) == 1){
...
//更新作品
function update_work($id, $title,$introduce, $image_path, $video_path, $publish){
    $result = null;
    $upload_date = date("Y-m-d H:i:s");
    ...
    $sql= "UPDATE work SET title='{$title}', title='{$introduce}', {$image_path_sql}, {$video_path_sql}, publish='${publish}', upload_date='{$upload_date}'
           WHERE id=$id";
    $query = $_SESSION['$link']->query($sql);
    if($query){
        if(mysqli_affected_rows($_SESSION['$link']) == 1){
